...
Module MainModule
    ...
    Sub ExploringProperties()
        ...
        For Each pi As PropertyInfo In GetType(TestClass).GetProperties(
    ...
    Sub BindingAGenericType()
        ...
        For Each argType As Type In genType.GetGenericArguments()
            If CBool(argType.GenericParameterAttributes And GenericParameterAttributes.DefaultConstructorConstraint) Then
                Console.WriteLine("  New (default constructor)")
            End If
            If CBool(argType.GenericParameterAttributes And GenericParameterAttributes.ReferenceTypeConstraint) Then
                Console.WriteLine("  Class (reference type)")
            End If
            If CBool(argType.GenericParameterAttributes And GenericParameterAttributes.NotNullableValueTypeConstraint) Then
                Console.WriteLine("  Structure (nonnullable value type)")
            End If         ' Get the constraints for this argument.
            For Each constraint As Type In argType.GetGenericParameterConstraints()
                Console.WriteLine("  {0}", constraint.FullName)
            Next
    ...
    Sub ExploringAttributes()
        ...
        Dim miAttrs() As ConditionalAttribute = DirectCast(
    ...
    Sub ExploringAttributes2()
        ...
        Dim tyAttr As ObsoleteAttribute = DirectCast(Attribute.GetCustomAttribute(
    ...
    Sub CreatingAnObjectDinamically()
        ...
        Dim o3 As Object = type.InvokeMember("", BindingFlags.CreateInstance,
    ...
    Sub InvokeMemberMethod()
        ...
        Dim value As Object = type.InvokeMember("FirstName", BindingFlags.GetField,
           Nothing, obj, Nothing)
        ...
        type.InvokeMember("SendEmail", BindingFlags.InvokeMethod, Nothing, obj,
           New Object() {"This is a message", 2})

        ' Read the m_Age private field.
        Dim age As Object = type.InvokeMember("m_Age",
    ...
    Sub CovarianceAndContravariance()
        Dim pers As New Person()
        For Each ei As EventInfo In pers.GetType().GetEvents()
            Dim handlerType As Type = ei.EventHandlerType
            ' The universal event handler method
            Dim method As MethodInfo = GetType(MainModule).GetMethod("UniversalEventHandler")
            ' Leverage contravariance to create a delegate that points to the universal handler.
            Dim handler As [Delegate] = [Delegate].CreateDelegate(handlerType, Nothing, method)
            ' Use reflection to register the event.
            ei.AddEventHandler(pers, handler)
        Next
        ' Prove that it works.
        pers.SendEmail("an email", 1)
    End Sub

    Sub UniversalEventHandler(ByVal sender As Object, ByVal e As EventArgs)
        Console.WriteLine("An event has fired")
    End Sub

    Sub CreatingAUniversalComparer()
        Dim persons() As Person = {New Person("Joe", "Doe"),
           New Person("Ann", "Doe"),
    ...
    Sub SchedulingASequenceOfActions()
        ...
        Dim act As New Action("Create c:\backup directory", GetType(Directory),
           "CreateDirectory", "c:\backup")
        Dim undoAct As New Action("Delete c:\backup directory", GetType(Directory),
        ...
        act = New Action("Create the c:\myapp_readme.txt", GetType(File),
        ...
        act = New Action("Move the c:\myapp_readme.txt to c:\backup\readme.txt", GetType(File),
           "Move", "c:\myapp_readme.txt", "c:\backup\readme.txt")
        undoAct = New Action("Move c:\backup\readme.txt to c:\myapp_readme.txt", GetType(File),
        ...
        act = New Action("Copy c:\missing.txt to c:\backup", GetType(File),
    ...
    Sub PerformanceConsiderations()
        ...
        Dim mi As MethodInfo = GetType(String).GetMethod("IndexOf", BindingFlags.ExactBinding Or
    ...
    Sub SecurityIssues()
        ...
        Console.WriteLine("{0} {1}, spouse is {2} {3}", joe2.FirstName, joe2.LastName,
