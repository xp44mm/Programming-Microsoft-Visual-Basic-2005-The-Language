...
Module Module1
    ...
    Sub RegularExpressionOptions()
        ...
        Dim pattern As String =
           "\s*         # ignore leading spaces" + ControlChars.CrLf &
           "(           # two cases: quoted or unquoted string" + ControlChars.CrLf &
           "(?<quote>   # case 1: define a group named 'quote' '" + ControlChars.CrLf &
           "['""])     # the group is single or a double quote" + ControlChars.CrLf &
           ".*?         # a sequence of characters (lazy matching)" + ControlChars.CrLf &
           "\k<quote>   # followed by the same quote char" + ControlChars.CrLf &
           "|           # end of case 1" + ControlChars.CrLf &
           "[^'""]+     # case 2: a string without quotes" + ControlChars.CrLf &
           ")           # end of case 2" + ControlChars.CrLf &
    ...
    Sub TestReplaceWithCallback()
        ...
        Dim re2 As New Regex("\b(Usa|France|Germany|Italy|Great Britain)\b",
    ...
    Sub CompileToAssemblyMethod()
        ...
        Dim rci1 As New RegexCompilationInfo("\w+", RegexOptions.Compiled,
           "RegexWords", nsName, True)
        ' The second regular expression compiles to a type named RegexIntegers.
        Dim rci2 As New RegexCompilationInfo("\d+", RegexOptions.Compiled,
    ...
    Sub SearchingForWords()
        Dim text As String = "A word with 郼c閚t鑔 vowels, and the 123 number."
        Dim pattern As String
    ...
    Private Function ProximityMatches(ByVal text As String, ByVal word1 As String,
    ...
    Sub ParsingFixedWidthDataFiles()
        Dim text As String =
           "John  Smith   New York" & ControlChars.CrLf &
        ...
        For Each line As String In lines
            ...
            Console.WriteLine("First={0}, Last={1}, City={2}",
               m.Groups("first").Value.TrimEnd(), m.Groups("last").Value.TrimEnd(),
    ...
    Sub ParsingDelimitedQuotedDataFiles()
        Dim text As String =
           "'John, P.' , ""Smith"" , ""New York""" & ControlChars.CrLf &
        ...
        Dim pattern As String = "^\s*(?<q1>[""'])(?<first>.*?)\k<q1>\s*," &
    ...
    Sub ParsingDelimitedQuotedDataFiles2()
        Dim text As String =
           """John, P."" , Doe , ""Los Angeles, CA""" & ControlChars.CrLf &
        ...
        Dim pattern As String = "^\s*(?<q1>[""']?)(?<first>.*?)(?(q1)\k<q1>)\s*" &
           ",\s*(?<q2>[""']?)(?<last>.*?)(?(q2)\k<q2>)\s*" &
           ",\s*(?<q3>[""']?)(?<city>.*?)(?(q3)\k<q3>)\s*$"
